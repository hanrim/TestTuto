...
public class RequestFactory {
	...
	public ArrayList <String> host         = reqHost.host;
	...
	public byte [] sendHttpGetAsByte(){
		...
		try{
			...
			if(host != null){
			config.setHost(host.get(1), Integer.parseInt(host.get(2)));
			}
			host = reqHost.host;
			//Set Parameter Query
			String ParamQuery = "";
	...
	public String sendHttpGetAsString(){
		...
		try{
			...
			HostConfiguration config = client.getHostConfiguration();
			if(host != null){
			config.setHost(host.get(0), Integer.parseInt(host.get(1)));
			}
			host = reqHost.host;
			//Set Parameter Query
			String ParamQuery = "";
...
//	
//			//	host host = new host(host.Type.HTTP, new InetSocketAddress("168.219.61.252", 8080));
//			host host = new host(host.Type.HTTP, new InetSocketAddress("70.10.15.10", 8080));
//			
//			HttpURLConnection conn = (HttpURLConnection) url.openConnection(host);
...
//			System.out.println("host-Connection: "+ conn.getHeaderField("host-Connection"));
